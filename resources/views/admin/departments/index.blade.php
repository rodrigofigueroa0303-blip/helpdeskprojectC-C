  ...
  <div class="py-8">
    <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-6">
      ...
      <div class="card p-6 space-y-6 animate-slide-up">
        <div>
          <h3 class="text-xl font-bold text-brand tracking-tight">Listado de Departamentos</h3>
          <p class="text-gray-500 text-sm mt-0.5">Agrega o modifica las unidades operativas asignables a las solicitudes de soporte.</p>
        </div>

        <div class="bg-gray-50 p-4 rounded-xl border border-gray-100">
          <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">Crear nuevo departamento</h4>

          <form method="POST" action="{{ route('admin.departments.store') }}" class="flex items-center gap-4 flex-wrap">
            @csrf
            <div class="flex-1 min-w-[240px]">
              <input name="name" class="input font-medium text-gray-900" placeholder="Nombre del departamento (Ej. Infraestructura)" required>
            </div>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm text-left text-gray-600">
              <thead class="table-header">
                <tr>
                  <th class="table-cell">Nombre del Departamento</th>
              ...
              <tbody class="divide-y divide-gray-100 bg-white">
                @forelse($departments as $d)
                  <tr class="table-row">
                    <td class="table-cell font-bold text-gray-900">{{ $d->name }}</td>
                    ...
                    <td class="px-4 py-2 text-right">
                      <div class="flex items-center justify-end gap-3">
                        ...
                        <form method="POST" action="{{ route('admin.departments.update', $d) }}"
                          ...
                          <label class="inline-flex items-center gap-1 text-[11px] font-bold text-gray-500 cursor-pointer select-none px-1">
                            <input type="checkbox" name="is_active" value="1"
                              class="rounded border-gray-300 text-brand focus:ring-brand/30" @checked($d->is_active)>
                            Activo
                          </label>
                        ...
                        <form method="POST" action="{{ route('admin.departments.destroy', $d) }}"
                          onsubmit="return confirm('¿Estás seguro de que deseas eliminar este departamento?')">
                          @csrf
                          @method('DELETE')
                ...
                @empty
                  <tr>
                    <td colspan="3" class="px-4 py-8 text-center text-gray-400 italic">No hay departamentos configurados en la plataforma.</td>
                  </tr>
                @endforelse
              </tbody>
            </table>
